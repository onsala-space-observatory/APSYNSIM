...
# Try to read a default initial model:
    ...
    def GUI(self):  # ,makefigs=True):
        ...
        self.UVPlot = self.figUV.add_subplot(232,
                                             aspect='equal',
        ...
        self.spherePlotPlot = self.spherePlot.plot_surface(x,
                                                           y,
                                                           z,
                                                           rstride=4,
                                                           cstride=4,
        ...
        self.figUV.subplots_adjust(left=0.05,
                                   right=0.99,
                                   top=0.95,
                                   bottom=0.07,
                                   hspace=0.25)
        ...
        self.wax['gammacorr'] = pl.axes([0.46, 0.08, 0.13, 0.02],
                                        facecolor='white')
        self.wax['diameter'] = pl.axes([0.825, 0.08, 0.10, 0.02],
                                       facecolor='white')
        self.wax['subarrwgt'] = pl.axes([0.15, 0.58, 0.12, 0.02],
                                        facecolor='white')
        self.widget['robust'] = Slider(self.wax['robust'],
                                       r'Robust',
                                       -2.,
                                       2.,
                                       valinit=0.0)
        self.widget['lat'] = Slider(self.wax['lat'],
                                    r'Lat (deg)',
                                    -90.,
                                    90.,
                                    valinit=self.lat / self.deg2rad)
        self.widget['dec'] = Slider(self.wax['dec'],
                                    r'Dec (deg)',
                                    -90.,
                                    90.,
                                    valinit=self.dec / self.deg2rad)
        self.widget['H0'] = Slider(self.wax['H0'],
                                   r'H$_{0}$ (h)',
                                   -12.,
                                   12.,
                                   valinit=self.Hcov[0] / self.Hfac)
        self.widget['H1'] = Slider(self.wax['H1'],
                                   r'H$_{1}$ (h)',
                                   -12.,
                                   12.,
        ...
        self.widget['diameter'] = Slider(self.wax['diameter'],
                                         'Dish size (m)',
                                         0,
                                         100.,
                                         valinit=0.0,
        ...
        self.widget['subarrwgt'] = Slider(self.wax['subarrwgt'],
                                          'log(W1/W2)',
                                          -4,
                                          4,
                                          valinit=0,
...
#            wavelength = float(it[1])*1.e-3
    ...
    def _changeCoordinates(self, rescale=False, redoUV=False):
        ...
        newtext = self.fmtH % (self.lat / self.deg2rad, self.dec /
                               self.deg2rad, self.Hcov[0] / self.Hfac,
    ...
    def _setNoise(self, noise):
        if noise == 0.0:
            ...
            self.Noise[:] = np.random.normal(
                loc=0.0, scale=noise, size=np.shape(
                    self.Noise)) + 1.j * np.random.normal(
                        loc=0.0, scale=noise, size=np.shape(self.Noise))
        self._setBaselines()
        self._setBeam()
        self._plotBeam(redo=False)
        self._plotDirty(redo=False)
    ...
    def _setBaselines(self, antidx=-1):
        ...
        if self.Nant2 > 1:
            ...
            for currBas in bas2change:
                ...
                self.B2[currBas, 0] = -(
                    self.antPos2[n2][1] -
                    self.antPos2[n1][1]) * self.trlat[0] / self.wavelength[2]
                self.B2[currBas, 1] = (
                    self.antPos2[n2][0] -
                    self.antPos2[n1][0]) / self.wavelength[2]
                self.B2[currBas, 2] = (
                    self.antPos2[n2][1] -
                    self.antPos2[n1][1]) * self.trlat[1] / self.wavelength[2]
                self.u2[currBas, :] = -(self.B2[currBas, 0] * self.H[0] +
                                        self.B2[currBas, 1] * self.H[1])
                self.v2[currBas, :] = -self.B2[currBas, 0] * self.trdec[
                    0] * self.H[1] + self.B2[currBas, 1] * self.trdec[
                        0] * self.H[0] + self.trdec[1] * self.B2[currBas, 2]
    ...
    def _setBeam(self, antidx=-1):
        ...
        if self.Nant2 > 1:
            ...
            self.beam[:] += np.fft.ifftshift(
                np.fft.ifft2(np.fft.fftshift(
                    self.robustsamp2))).real * (self.W2W1 / (1. + self.W2W1))
            self.beamScale2 = np.max(self.beam[self.Nphf:self.Nphf +
    ...
    def _prepareModel(self):
        ...
        for model in self.models:
            ...
            if model[0] == 'D':
                ...
                if np.abs(xpix + self.Nphf) < self.Npix and np.abs(
                        ypix + self.Nphf) < self.Npix:
                    yint = ypix + self.Nphf
                    xint = xpix + self.Nphf
    ...
    def _plotModel(self, redo=True):
        ...
        if redo:
            ...
            self.modelPlotPlot = self.modelPlot.imshow(
                ...
                vmin=0.0,
                vmax=np.max(self.modelimTrue)**self.gamma,
                cmap=self.currcmap)

            modflux = self.modelimTrue[self.Nphf, self.Nphf]
            self.modelText = self.modelPlot.text(
                0.05,
                0.87,
        ...
        self.totflux = np.sum(self.modelimTrue[Np4:self.Npix -
                                               Np4, Np4:self.Npix - Np4])
    ...
    def _plotModelFFT(self, redo=True):
        ...
        if redo:
            mymap = pl.gray()
            self.UVPlotFFTPlot = self.UVPlot.imshow(toplot,
                                                    cmap=mymap,
                                                    vmin=0.0,
                                                    vmax=Mval + dval,
                                                    picker=5)
    ...
    #  print 'RMS2: ',np.std(np.abs(self.dirtymap[:]))  #, self.beamScale
        ...
        if redo:
            ...
            self.dirtyText = self.dirtyPlot.text(
                0.05,
                0.87,
                self.fmtD % (modflux, 0.0, 0.0),
                transform=self.dirtyPlot.transAxes,
                bbox=dict(facecolor='white', alpha=0.7))
            pl.setp(self.dirtyPlotPlot,
                    extent=(self.Xaxmax / 2., -self.Xaxmax / 2.,
                            -self.Xaxmax / 2., self.Xaxmax / 2.))
            self.curzoom[1] = (self.Xaxmax / 2., -self.Xaxmax / 2.,
                               -self.Xaxmax / 2., self.Xaxmax / 2.)
    ...
    def _plotAntennas(self, redo=True, rescale=False):
        ...
        if redo:
            ...
            self.antPlotPlot = self.antPlot.plot(toplot[:, 0],
                                                 toplot[:, 1],
                                                 'o',
                                                 color='lime',
                                                 picker=5)[0]
            if self.Nant2 > 1:
                ...
                self.antPlotPlot2 = self.antPlot.plot(toplot2[:, 0],
                                                      toplot2[:, 1],
                                                      'or',
                                                      picker=5)[0]
            ...
            self.antText = self.antPlot.text(0.05,
                                             0.88,
                                             self.fmtA %
            ...
            self.UVPlotPlot.append(
                self.UVPlot.plot(toplotu,
                                 toplotv,
                                 '.',
                                 color='lime',
                                 markersize=1,
            ...
            self.UVPlot.set_ylim(
                (2. * self.Xmax / self.wavelength[2] / self.lfac,
                 -2. * self.Xmax / self.wavelength[2] / self.lfac))
            self.curzoom[2] = (2. * self.Xmax / self.lfac,
                               -2. * self.Xmax / self.lfac,
                               2. * self.Xmax / self.lfac,
                               -2. * self.Xmax / self.lfac)
            self.latText = self.UVPlot.text(
                0.05,
                0.87,
                self.fmtH %
            ...
            self.basText = self.UVPlot.text(0.05,
                                            0.02,
                                            self.fmtBas % (0, 0, 0.0),
                                            transform=self.UVPlot.transAxes)
            self.antPlotBas.set_data([[0], [0]])

            self.visText = self.UVPlot.text(0.05,
                                            0.08,
                                            self.fmtVis % (0.0, 0.0),
                                            transform=self.UVPlot.transAxes)
            ...
            if rescale:
                ...
                self.curzoom[2] = (2. * self.Xmax / self.lfac,
                                   ...
                                   2. * self.Xmax / self.lfac,
                                   -2. * self.Xmax / self.lfac)
                self.curzoom[3] = (-self.Xmax, self.Xmax, -self.Xmax,
                                   self.Xmax)
    ...
    def _plotBeam(self, redo=True):
        ...
        if redo:
            ...
            self.beamPlotPlot = self.beamPlot.imshow(
                self.beam[Np4:self.Npix - Np4, Np4:self.Npix - Np4],
                picker=True,
                interpolation='nearest',
                cmap=self.currcmap)
            ...
            self.beamPlotPlot.set_data(self.beam[Np4:self.Npix -
                                                 Np4, Np4:self.Npix - Np4])
            self.beamText.set_text(self.fmtB % (1.0, 0.0, 0.0))
    ...
    def _onPick(self, event):
        ...
        if event.mouseevent.inaxes == self.UVPlot:
            ...
            if event.artist in self.UVPlotPlot:
                ...
                self.antPlotBas.set_data(
                    [[self.antPos[n1][0], self.antPos[n2][0]],
                     [self.antPos[n1][1], self.antPos[n2][1]]])
                ...
                H = self.Horig[idata[1][0]] / self.Hfac
                newtext = self.fmtBas % (n1 + 1 + self.Nant,
                                         n2 + 1 + self.Nant, H)
                self.basText.set_text(newtext)
                self.antPlotBas.set_data(
                    [[self.antPos2[n1][0], self.antPos2[n2][0]],
                     [self.antPos2[n1][1], self.antPos2[n2][1]]])
...
# Drag the sphere plot (to change source position)
        if self._onSphere:
            ...
            if not self.lock:
                self.lock = True
                if newDec != self.dec and np.abs(newDec -
                                                 self.lat) < np.pi / 2.:
                    self.widget['dec'].set_val(newDec / self.deg2rad)
                    self.spherePlot.view_init(elev=newDec / self.deg2rad,
                                              azim=0.0)
                    self.dec = newDec
...
# ZOOM IN:
            ...
            if cz >= 0:
                if event.button == 1 and cz >= 0:
                    RA = event.xdata
                    Dec = event.ydata
                    xL = np.abs(self.curzoom[cz][1] -
                                self.curzoom[cz][0]) / 4. / scal
                    yL = np.abs(self.curzoom[cz][3] -
                                self.curzoom[cz][2]) / 4. / scal
                    x0 = RA - xL
                    x1 = RA + xL
                    y0 = Dec - xL
                    y1 = Dec + xL
...
# ZOOM OUT:
                if event.button == 3:
                    ...
                    Dec = event.ydata
                    xL = np.abs(self.curzoom[cz][1] -
                                self.curzoom[cz][0]) / scal
                    yL = np.abs(self.curzoom[cz][3] -
                                self.curzoom[cz][2]) / scal
                    if cz in [0, 1]:
                        if xL > self.Xaxmax / 2.:
                            xL = self.Xaxmax / 2.
                        if yL > self.Xaxmax / 2.:
                ...
                self.curzoom[cz] = (xx0 * scal, xx1 * scal, yy0 * scal,
                                    yy1 * scal)
...
class CLEANer(object):
    ...
    def __init__(self, parent):
        ...
        self.entries['H0'] = Tk.Scale(self.frames['H0Fr'],
                                      ...
                                      length=200)
        self.entries['H1'] = Tk.Scale(self.frames['H1Fr'],
                                      from_=0,
                                      to=self.parent.nH,
                                      orient=Tk.HORIZONTAL,
                                      length=200)
    ...
    def _doRestore(self):
        ...
        if self.dorestore:
            ...
            toadd = self.cleanmodd[self.Np4:self.parent.Npix -
                                   self.Np4, self.Np4:self.parent.Npix -
                                   self.Np4]
            ...
            if self.resadd:
                toadd = (self.cleanmod +
                         self.residuals)[self.Np4:self.parent.Npix -
                                         self.Np4, self.Np4:self.parent.Npix -
                                         self.Np4]
            else:
                toadd = self.cleanmod[self.Np4:self.parent.Npix -
                                      self.Np4, self.Np4:self.parent.Npix -
                                      self.Np4]
    ...
    def _ApplyGain(self):
        ...
        G = float(self.entries['Amp'].get()) / 100. * np.exp(
            1.j * float(self.entries['Phas'].get()) * np.pi / 180.)
    ...
    def _onPick(self, event):
        ...
        self.ResidText.set_text(self.fmtD2 %
                                (Flux, RA, Dec, self.PEAK, self.RMS))
        if self.dorestore:
            if self.resadd:
        ...
        self.CLEANText.set_text(
            self.fmtDC %
            (Flux, RA, Dec, self.CLEANPEAK, self.CLEANPEAK / self.RMS) + '\n' +
            self.Beamtxt)
    ...
    def _onRelease(self, event):
        ...
        if self.moved:
            ...
            self.CLEANPlot.set_ylim(
                (self.parent.curzoom[1][2], self.parent.curzoom[1][3]))
            self.canvas1.draw()
    ...
    def _doMask(self, event):
        if self.pressed >= 0 and event.inaxes == self.ResidPlot:
            ...
            x1 = np.floor(
                (self.Xaxmax - Dec) / (2. * self.Xaxmax) * self.parent.Npix)
            self.Box.set_data(
                [self.xydata[0], self.xydata[0], RA, RA, self.xydata[0]],
                [self.xydata[1], Dec, Dec, self.xydata[1], self.xydata[1]])
            self.canvas1.draw()
    ...
    def _reset(self, donoise=False):
        ...
        dslice = self.parent.dirtymap[self.Np4:self.parent.Npix -
                                      self.Np4, self.Np4:self.parent.Npix -
                                      self.Np4]
        ...
        self.ResidText = self.ResidPlot.text(
            0.05,
            0.87,
            self.fmtD2 % (modflux, 0.0, 0.0, self.PEAK, self.RMS),
            transform=self.ResidPlot.transAxes,
            bbox=dict(facecolor='white', alpha=0.7))
        pl.setp(self.ResidPlotPlot,
                extent=(self.parent.Xaxmax / 2., -self.parent.Xaxmax / 2.,
                        -self.parent.Xaxmax / 2., self.parent.Xaxmax / 2.))
        ...
        self.CLEANPlotPlot = self.CLEANPlot.imshow(
            self.parent.dirtymap[self.Np4:self.parent.Npix -
                                 self.Np4, self.Np4:self.parent.Npix -
                                 self.Np4],
            interpolation='nearest',
            picker=True,
            cmap=self.parent.currcmap)
        modflux = self.parent.dirtymap[self.parent.Nphf, self.parent.Nphf]
        self.CLEANText = self.CLEANPlot.text(
            0.05,
            0.83,
            self.fmtDC % (0.0, 0.0, 0.0, 0., 0.),
            transform=self.CLEANPlot.transAxes,
            bbox=dict(facecolor='white', alpha=0.7))
        pl.setp(self.CLEANPlotPlot,
                extent=(self.parent.Xaxmax / 2., -self.parent.Xaxmax / 2.,
                        -self.parent.Xaxmax / 2., self.parent.Xaxmax / 2.))
            ...
            try:
                fit = spfit.leastsq(
                    lambda x: np.exp(-(dX * dX * x[0] + dY * dY * x[
                        1] + dX * dY * x[2])) - self.parent.beam[MainLobe],
                    [1., 1., 0.])
                ...
                A = 2.355 * (
                    2. /
                    (ApB + AmB))**0.5 * self.parent.imsize / self.parent.Npix
                B = 2.355 * (
                    2. /
                    (ApB - AmB))**0.5 * self.parent.imsize / self.parent.Npix
                if A < B:
                ...
                if B > 0.1:
                    self.Beamtxt = '%.1f x %.1f as (PA = %.1f deg.)' % (A, B,
                                                                        Pang)
                else:
    ...
    def _CLEAN(self):
        ...
        for i in range(niter):
            ...
            self.CLEANPlot.set_title('CLEAN (%i ITER): %.2e Jy' %
            ...
            self.CLEANPlotPlot.set_array(
                toadd[self.Np4:self.parent.Npix -
                      self.Np4, self.Np4:self.parent.Npix - self.Np4])
            ...
            self.CLEANText.set_text(
                self.fmtDC %
                (clFlux, RA, Dec, self.CLEANPEAK, self.CLEANPEAK / self.RMS) +
                '\n' + self.Beamtxt)
...
#      pass
    ...
    def _convSource(self):
        ...
        self.CSText = self.CS1.text(0.05,
                                    0.87,
    ...
    def _CSRead(self):
        ...
        self.CSImage = (np.fft.fftshift(
            np.fft.ifft2(self.parent.modelfft *
                         np.fft.fft2(np.fft.fftshift(self.cleanBeam))))).real

        self.CS1Plot = self.CS1.imshow(
            self.CSImage[self.Np4:self.parent.Npix -
                         self.Np4, self.Np4:self.parent.Npix - self.Np4],
            interpolation='nearest',
            picker=True,
            cmap=self.parent.currcmap)
        ...
        Toplot = np.abs(
            np.fft.fftshift(np.fft.fft2(np.fft.fftshift(self.residuals))))
...
class UVPLOTTER2(object):
    ...
    def _FFTRead(self):
        ...
        Toplot = np.abs(
            np.fft.fftshift(np.fft.fft2(np.fft.fftshift(
                self.parent.dirtymap))))

        vmax = np.max(Toplot)
        vmin = 0.0

        self.UVOBSPlot = self.UVOBS.imshow(Toplot,
                                           vmin=0.0,
                                           vmax=vmax,
                                           cmap=self.parent.currcmap,
                                           picker=True,
        ...
        Toplot = np.abs(
            np.fft.fftshift(
                np.fft.fft2(np.fft.fftshift(self.parent.modelimTrue))))
    ...
    def _onUVPick(self, event):
        ...
        yi = np.floor((self.parent.UVmax + Up) / (self.parent.UVmax) *
                      self.parent.Npix / 2.)
        xi = np.floor((self.parent.UVmax - Vp) / (self.parent.UVmax) *
                      self.parent.Npix / 2.)
        ...
        if xi > 0 and yi > 0 and xi < self.parent.Npix and yi < self.parent.Npix:
            ...
            self.UVOBSText.set_text(self.UVfmt %
                                    self.UVOBSPlot.get_array()[xi, yi])
        else:
